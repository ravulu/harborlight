...
export const FAQ: Qa[] = [
  ...
  {
    q: 'Should I use the debt snowball or the debt avalanche?',
    a: 'Both work the same way: you pay the minimum on every debt, put whatever is spare at one of them, and when that one clears its whole payment joins the spare money and goes at the next. Each debt that falls makes the next fall faster. They differ in one thing only — which debt you go at first. The avalanche goes at the highest interest rate, and always costs less interest, because interest is charged on what you still owe and that is where the most of it is building up. The snowball goes at the smallest balance, and gets one debt gone sooner, often much sooner; people who have tried both often say that is what kept them going. Which of those matters more is a fact about you rather than about the arithmetic, so the debt payoff calculator prices both on your own figures, shows when each debt would go under each, and names no winner. Two things surprise people. The two often come out identical, because the smallest balance frequently carries the highest rate — a store card against a mortgage. And if you have not entered an interest rate on each debt they cannot differ at all, because there is nothing for the avalanche to sort by.',
  },
  {
    q: 'Does paying off debt change my retirement projection?',
    a: 'Not yet, and it is worth knowing before you wonder why the chart did not move. The projection models what you put in and what you draw out; it does not model debt payments, so clearing a card does not change the balance chart, the confidence figure, or the year the money runs out. The debt payoff calculator is a separate tool for that reason — it answers how fast the debts clear, not what that does to retirement. This is the same gap the assets and liabilities tab has: what you own and owe is held and taxed carefully and none of it reaches the projection. Where debt does count today is net worth, which subtracts what you owe from what you own.',
  },
  {
  ...
  },
  {
    ...
    a: 'Throughout, and it shows the results in today’s dollars so the figures mean something. Spending is held level in real terms, which means it rises in the dollars of the day; Social Security drifts by the gap between its cost-of-living adjustment and your inflation rate. Tax brackets and the standard deduction are the published figures for as long as those exist, and beyond them are carried forward at an assumed rate — the page names the year it is using and says when that year is an estimate rather than a published table. One thing is deliberately not held level: Medicare premiums are grown faster than prices, because they have outrun inflation for most of the past decade. That gap is faded out over twenty years rather than compounded for the whole plan — a rate that outruns prices for a decade is history, and one that does it for fifty years is a claim nobody makes — after which the surcharge rises with its thresholds and stops gaining on them. So a lifetime surcharge is larger in today’s money than today’s rates would suggest, and on a long plan it is a good deal smaller than compounding that gap would suggest. The insight that reports it says by how much. Where a figure is shown in the dollars of a future year rather than today’s, the page says so.',
  },
  {
